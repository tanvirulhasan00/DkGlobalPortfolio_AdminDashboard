...
import { updateAuthor } from "~/redux/features/blogSlice";
...
const AuthorUpdate = () => {
  ...
  const { loading, data: blogData } = useAppSelector((state) => state.blog);
  ...
  useEffect(() => {
    ...
    const ShowToast = blogData?.success ? toast.success : toast.error;
    ShowToast(blogData?.statusCode ?? blogData?.message, {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Also append the original avatar URL if no new file is selected
    if (!selectedFile && data.avatar) {
      formPayload.append("avatar", data.avatar);
    }
    // Handle form submission
    dispatch(updateAuthor({ token: "", formPayload }));
      ...
      <div className={`bg-white rounded-2xl shadow-sm border border-gray-200`}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end space-x-3 p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
            ...
            >
              {loading ? <LoadingSpinner /> : "Update Author"}
            </button>
          </div>
        </form>
      </div>
