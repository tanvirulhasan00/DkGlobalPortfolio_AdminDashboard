...
import { createCategory } from "~/redux/features/blogSlice";
import { useAppDispatch, useAppSelector } from "~/redux/hooks/hook";
import { getToken } from "~/components/route-components/getLocalStorage";

const BlogCategoryCreatePage = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch(createCategory({ token: token || "", formPayload }));
    ...
    <div className="container mx-auto p-6">
      <div className={`bg-white rounded-2xl shadow-sm border border-gray-200`}>
        <div className="flex items-center space-x-3 p-6 border-b border-gray-100">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Add Blog Category</h2>
            <p className="text-sm text-gray-500">Add a new blog category</p>
          </div>
        </div>
