...
import { updateProductCat } from "~/redux/features/productSlice";
import { useAppDispatch, useAppSelector } from "~/redux/hooks/hook";

const ProductCategoryUpdatePage = () => {
  const location = useLocation();
  const data = location.state || {};
  const dispatch = useAppDispatch();
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isAttempted, setIsAttempted] = useState<boolean>(true);
  const [preview, setPreview] = useState(data?.icon || "/preview.avif");
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch(updateProductCat({ token: "", formPayload }));
    ...
    <div className="container mx-auto p-6">
      <div className={`bg-white rounded-2xl shadow-sm border border-gray-200`}>
        <div className="flex items-center space-x-3 p-6 border-b border-gray-100">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Update Product Category</h2>
            <p className="text-sm text-gray-500">Modify the product category details</p>
          </div>
        </div>
